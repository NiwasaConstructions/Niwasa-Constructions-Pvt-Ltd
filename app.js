...
function initDataLoad() {
    ...
    onSnapshot(collection(db, "vendors"), (snap) => {
        ...
        snap.forEach(docSnap => {
            let d = docSnap.data();
            let bankInfo = d.bank_name ? `${d.bank_name}<br><small>${d.bank_acc}</small>` : '-';
            vTable.innerHTML += `<tr><td><span class="badge bg-secondary">${d.custom_id}</span></td><td class="fw-bold">${d.name}</td><td>${d.contact}<br><small class="text-muted">${d.address || ''}</small></td><td>${bankInfo}</td></tr>`;
            dropHtml += `<option value="${docSnap.id}">${d.custom_id} - ${d.name}</option>`;
        });
        vDrops.forEach(drop => drop.innerHTML = dropHtml);
    });

    // Sites (Handles Ongoing vs Completed)
    onSnapshot(collection(db, "sites"), (snap) => {
        const sTable = document.getElementById('s-table');
        const activeDrops = document.querySelectorAll('.active-site-dropdown'); // For Bill Entry
        const allDrops = document.querySelectorAll('.report-site-dropdown'); // For Reports
        ...
        snap.forEach(docSnap => {
            ...
            let btnClass = isOngoing ? 'btn-outline-success' : 'btn-secondary';
            let btnText = isOngoing ? 'Mark Completed' : 'Completed';
            
            sTable.innerHTML += `<tr>
                <td class="fw-bold"><i class="bi bi-geo-alt text-danger me-2"></i> ${d.name}</td>
                <td><span class="badge bg-${isOngoing ? 'primary' : 'secondary'}">${d.status}</span></td>
                <td><button class="btn btn-sm ${btnClass} toggle-site-btn" data-id="${docSnap.id}" data-status="${d.status}" ${!isOngoing ? 'disabled' : ''}>${btnText}</button></td>
            </tr>`;
            
            allHtml += `<option value="${docSnap.id}">${d.name}</option>`;
            if(isOngoing) activeHtml += `<option value="${docSnap.id}">${d.name}</option>`;
        });
        activeDrops.forEach(drop => drop.innerHTML = activeHtml);
        allDrops.forEach(drop => drop.innerHTML = allHtml);

        document.querySelectorAll('.toggle-site-btn').forEach(btn => btn.addEventListener('click', async (e) => {
            if(confirm("Mark this site as Completed? It won't appear in new bill entries.")) await updateDoc(doc(db, "sites", e.currentTarget.dataset.id), { status: "COMPLETED" });
        }));
    });
    ...
    onSnapshot(collection(db, "bills"), (snap) => {
        ...
        snap.forEach(docSnap => {
            let d = docSnap.data(); allBillsList.push({ id: docSnap.id, ...d });
    ...
    onSnapshot(collection(db, "payments"), (snap) => {
        allPaymentsList = []; let monthlyPaid = 0, pendingChequesTotal = 0;
        ...
        snap.forEach(docSnap => {
            ...
            // Render Payment History Main Table
            ...
            pTable.innerHTML += `<tr>
                <td>${d.payment_date}</td><td class="fw-bold">${d.payment_id}</td>
                <td>${d.method.replace('_', ' ')} ${d.cheque_number ? `(${d.cheque_number})` : ''}</td>
...
document.getElementById('vendor-form').addEventListener('submit', async (e) => {
    ...
    await setDoc(doc(collection(db, "vendors")), { 
        custom_id: generateCustomID('VEN'), name: document.getElementById('v-name').value, 
        contact: document.getElementById('v-contact').value, address: document.getElementById('v-address').value,
        bank_name: document.getElementById('v-bank-name').value, bank_acc: document.getElementById('v-acc-no').value
    });
...
document.getElementById('site-form').addEventListener('submit', async (e) => {
    e.preventDefault();
    await setDoc(doc(collection(db, "sites")), { name: document.getElementById('s-name').value, status: 'ONGOING' });
    e.target.reset(); bootstrap.Modal.getInstance(document.getElementById('siteModal')).hide();
});

// Bill (With Duplicate Check)
document.getElementById('bill-form').addEventListener('submit', async (e) => {
    e.preventDefault();
    const vId = document.getElementById('b-vendor').value; const bNo = document.getElementById('b-number').value;
    
    // Duplicate Check
    const isDuplicate = allBillsList.some(b => b.vendor_id === vId && b.bill_number.toLowerCase() === bNo.toLowerCase());
    if(isDuplicate) return alert("Error: This Bill Number already exists for this vendor!");

    const btn = document.getElementById('b-submit-btn'); const fileInput = document.getElementById('b-file');
    let imageUrl = null;
    ...
    await setDoc(doc(collection(db, "bills")), {
        vendor_id: vId, site_id: document.getElementById('b-site').value, items_info: document.getElementById('b-items').value,
        bill_number: bNo, date: document.getElementById('b-date').value, total_amount: total,
        paid_amount: type === 'CASH' ? total : 0, status: type === 'CASH' ? 'SETTLED' : 'PENDING',
        attachment_url: imageUrl, created_at: new Date().toISOString()
    });
    
    alert("Bill Saved!"); e.target.reset(); document.getElementById('b-date').value = new Date().toISOString().split('T')[0];
    btn.innerHTML = `Save Bill`; btn.disabled = false;
...
function applyBillFilters() {
    ...
    filtered.forEach(d => {
        ...
        bTable.innerHTML += `<tr>
            ...
            <td>${imgBtn}</td><td><button class="btn btn-sm btn-outline-danger del-bill-btn" data-id="${d.id}" data-paid="${d.paid_amount}"><i class="bi bi-trash"></i></button></td>
        </tr>`;
    });
    document.querySelectorAll('.del-bill-btn').forEach(btn => btn.addEventListener('click', async (e) => {
        if(Number(e.currentTarget.dataset.paid) > 0) return alert("Cannot delete! Payments are attached.");
...
document.getElementById('p-vendor').addEventListener('change', (e) => {
    ...
    if(pendingBills.length === 0) { pbTable.innerHTML = '<tr><td colspan="3" class="text-center text-danger">No pending bills.</td></tr>'; } 
    else {
        pendingBills.forEach(d => {
            let due = d.total_amount - d.paid_amount;
            pbTable.innerHTML += `<tr><td class="fw-bold">${d.bill_number} <br><span class="badge bg-secondary">${d.items_info || '-'}</span></td>
                <td class="text-danger fw-bold">Rs. ${due}</td><td><input type="number" class="form-control pay-input border-primary" data-id="${d.id}" max="${due}" min="0"></td></tr>`;
        });
    }
...
document.getElementById('generate-rep-btn').addEventListener('click', () => {
    ...
    if(repType === 'SITE') {
        ...
        if(startDt) {
            lTable.innerHTML += `<tr class="table-warning"><td colspan="3" class="fw-bold">Brought Forward (B/F) Balance</td><td>-</td><td>-</td><td class="fw-bold text-danger">${bfBalance.toLocaleString()}</td><td>-</td></tr>`;
        }
        ...
        ledgerData.forEach(item => {
            ...
            lTable.innerHTML += `<tr>
                ...
                <td class="fw-bold">${runningBalance.toLocaleString()}</td><td class="text-center">${item.attach}</td>
            </tr>`;
        });
