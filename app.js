...
async function uploadToImgBB(file) {
    ...
    } catch (error) { console.error("Image processing error:", error); throw error; }
}
...
onAuthStateChanged(auth, (user) => {
    if (user) {
        ...
        const today = new Date().toISOString().split('T')[0]; document.getElementById('b-date').value = today; document.getElementById('p-date').value = today; document.getElementById('l-date').value = today;
        initDataLoad();
    } else {
        document.getElementById('login-section').style.display = 'block'; document.getElementById('dashboard-section').style.display = 'none';
    }
});
...
document.querySelectorAll('.nav-link').forEach(link => {
    link.addEventListener('click', (e) => {
        e.preventDefault(); document.querySelectorAll('.nav-link').forEach(l => l.classList.remove('active')); e.currentTarget.classList.add('active');
        document.querySelectorAll('.app-section').forEach(sec => sec.classList.remove('active')); document.getElementById(`section-${e.currentTarget.getAttribute('data-target')}`).classList.add('active');
    });
});
...
function renderPagination(sectionKey, dataLength, renderFunction) {
    const state = paginationStates[sectionKey]; const container = document.getElementById(`pg-${sectionKey}`); if(!container) return;
    if (state.pageSize === 'ALL') { container.innerHTML = ''; return; }
    const totalPages = Math.ceil(dataLength / state.pageSize); if(state.currentPage > totalPages) state.currentPage = totalPages > 0 ? totalPages : 1;
    const startItem = dataLength === 0 ? 0 : ((state.currentPage - 1) * state.pageSize) + 1; const endItem = Math.min(state.currentPage * state.pageSize, dataLength);

    container.innerHTML = `<div class="page-info">Showing ${startItem} to ${endItem} of ${dataLength} entries</div><div><button class="btn btn-sm btn-outline-secondary me-1" id="btn-prev-${sectionKey}" ${state.currentPage === 1 ? 'disabled' : ''}>Previous</button><span class="mx-2 fw-bold text-primary">Page ${state.currentPage} of ${totalPages || 1}</span><button class="btn btn-sm btn-outline-secondary ms-1" id="btn-next-${sectionKey}" ${state.currentPage >= totalPages ? 'disabled' : ''}>Next</button></div>`;

    const prevBtn = document.getElementById(`btn-prev-${sectionKey}`); if(prevBtn) prevBtn.addEventListener('click', () => { if(state.currentPage > 1) { state.currentPage--; renderFunction(); } });
    const nextBtn = document.getElementById(`btn-next-${sectionKey}`); if(nextBtn) nextBtn.addEventListener('click', () => { if(state.currentPage < totalPages) { state.currentPage++; renderFunction(); } });
}

['vendors', 'sites', 'bills', 'payments', 'cheques', 'lorry'].forEach(key => {
    const selectEl = document.getElementById(`page-size-${key}`);
    if(selectEl) { selectEl.addEventListener('change', (e) => { let val = e.target.value; paginationStates[key].pageSize = val === 'ALL' ? 'ALL' : parseInt(val); paginationStates[key].currentPage = 1; if(key === 'vendors') renderVendorsTable(); if(key === 'sites') renderSitesTable(); if(key === 'bills') applyBillFilters(); if(key === 'payments') renderPaymentsTable(); if(key === 'cheques') renderChequesTable(); if(key === 'lorry') renderLorryTable(); }); }
});
...
function renderVendorFilterButtons() {
    ...
    document.querySelectorAll('.vendor-filter-btn').forEach(btn => { btn.addEventListener('click', (e) => { currentBillVendorFilter = e.currentTarget.dataset.id; paginationStates.bills.currentPage = 1; renderVendorFilterButtons(); applyBillFilters(); }); });
}
...
function renderVendorsTable() {
    const vTable = document.getElementById('v-table'); vTable.innerHTML = ''; const state = paginationStates.vendors; let dataToRender = allVendorsList;
    if (state.pageSize !== 'ALL') { const start = (state.currentPage - 1) * state.pageSize; dataToRender = allVendorsList.slice(start, start + state.pageSize); }
    dataToRender.forEach(d => {
        let bankInfo = d.bank_name ? `<strong>${d.bank_name}</strong> ${d.bank_branch ? `(${d.bank_branch})` : ''}<br><small>${d.acc_name || ''} - ${d.bank_acc || ''}</small>` : '-';
        vTable.innerHTML += `<tr><td><span class="badge bg-secondary">${d.custom_id}</span></td><td class="fw-bold">${d.name}</td><td>${d.contact}<br><small class="text-muted">${d.address || ''}</small></td><td>${bankInfo}</td><td><button class="btn btn-sm btn-outline-primary edit-vendor-btn" data-id="${d.id}"><i class="bi bi-pencil"></i></button></td></tr>`;
    });
...
function renderSitesTable() {
    const sTable = document.getElementById('s-table'); sTable.innerHTML = ''; const state = paginationStates.sites; let dataToRender = currentSitesList;
    if (state.pageSize !== 'ALL') { const start = (state.currentPage - 1) * state.pageSize; dataToRender = currentSitesList.slice(start, start + state.pageSize); }
    dataToRender.forEach(d => {
        let isOngoing = d.status === 'ONGOING';
        sTable.innerHTML += `<tr><td class="fw-bold"><i class="bi bi-geo-alt text-danger me-2"></i> ${d.name}</td><td><span class="badge bg-${isOngoing ? 'primary' : 'secondary'}">${d.status}</span></td><td><button class="btn btn-sm ${isOngoing ? 'btn-outline-success' : 'btn-secondary'} toggle-site-btn" data-id="${d.id}" data-status="${d.status}" ${!isOngoing ? 'disabled' : ''}>${isOngoing ? 'Mark Completed' : 'Completed'}</button></td></tr>`;
    });
    document.querySelectorAll('.toggle-site-btn').forEach(btn => btn.addEventListener('click', async (e) => { if(confirm("Mark this site as Completed?")) await updateDoc(doc(db, "sites", e.currentTarget.dataset.id), { status: "COMPLETED" }); }));
    renderPagination('sites', currentSitesList.length, renderSitesTable);
}

function renderPaymentsTable() {
    const pTable = document.getElementById('payment-history-table'); pTable.innerHTML = ''; const state = paginationStates.payments; let dataToRender = allPaymentsList;
    if (state.pageSize !== 'ALL') { const start = (state.currentPage - 1) * state.pageSize; dataToRender = allPaymentsList.slice(start, start + state.pageSize); }
    dataToRender.forEach(d => {
        ...
        pTable.innerHTML += `<tr><td>${d.payment_date}</td><td class="fw-bold">${d.payment_id}</td><td>${d.method.replace('_', ' ')} ${d.cheque_number ? `(${d.cheque_number})` : ''}</td><td class="text-success fw-bold">Rs. ${d.total_amount.toLocaleString()}</td><td>${statusBadge}</td><td>${imgBtn}</td><td><button class="btn btn-sm btn-outline-info view-payment-btn" data-id="${d.id}"><i class="bi bi-eye"></i></button></td></tr>`;
    });
    document.querySelectorAll('.view-payment-btn').forEach(btn => {
        btn.addEventListener('click', (e) => {
            ...
            document.getElementById('detail-pay-id').innerText = payData.payment_id; document.getElementById('detail-pay-date').innerText = payData.payment_date; document.getElementById('detail-pay-method').innerText = payData.method.replace('_', ' '); document.getElementById('detail-pay-total').innerText = 'Rs. ' + payData.total_amount.toLocaleString();
            if(payData.cheque_number) { document.getElementById('detail-cheque-info').style.display = 'block'; document.getElementById('detail-cheque-text').innerText = `${payData.cheque_number} (Date: ${payData.cheque_date || '-'} | Status: ${payData.cheque_status})`; } else { document.getElementById('detail-cheque-info').style.display = 'none'; }
            const allocTable = document.getElementById('detail-allocations-table'); allocTable.innerHTML = '';
            if(payData.allocations && payData.allocations.length > 0) { payData.allocations.forEach(alloc => { const bill = allBillsList.find(b => b.id === alloc.bill_id); allocTable.innerHTML += `<tr><td class="fw-bold">${bill ? bill.bill_number : '<span class="text-danger">Deleted Bill</span>'}</td><td>${bill ? (bill.items_info || '-') : '-'}</td><td class="text-success fw-bold">Rs. ${alloc.amount.toLocaleString()}</td></tr>`; }); } else { allocTable.innerHTML = `<tr><td colspan="3" class="text-center text-muted">No specific bills allocated.</td></tr>`; }
...
function renderLorryTable() {
    const lTable = document.getElementById('lorry-table'); lTable.innerHTML = ''; const state = paginationStates.lorry; let dataToRender = allLorryRecords; 
    if (state.pageSize !== 'ALL') { const start = (state.currentPage - 1) * state.pageSize; dataToRender = allLorryRecords.slice(start, start + state.pageSize); }
    dataToRender.forEach(d => {
        let imgBtn = d.attachment_url ? `<a href="${d.attachment_url}" target="_blank" class="text-primary"><i class="bi bi-image fs-5"></i></a>` : '-';
        let inAmt = (d.type === 'ADVANCE' || d.type === 'OPENING_BAL') ? d.amount.toLocaleString() : '-'; let outAmt = d.type === 'EXPENSE' ? d.amount.toLocaleString() : '-'; let badge = d.type === 'OPENING_BAL' ? '<span class="badge bg-dark">Opening</span> ' : '';
    ...
    document.querySelectorAll('.del-lorry-btn').forEach(btn => btn.addEventListener('click', async (e) => { if(confirm("Delete this record?")) await deleteDoc(doc(db, "lorry_cash", e.currentTarget.dataset.id)); }));
...
function initDataLoad() {
    ...
    onSnapshot(collection(db, "sites"), (snap) => {
        ...
        snap.forEach(docSnap => {
            let d = docSnap.data(); d.id = docSnap.id; currentSitesList.push(d); let isOngoing = d.status === 'ONGOING';
            allHtml += `<option value="${docSnap.id}">${d.name}</option>`; if(isOngoing) activeHtml += `<option value="${docSnap.id}">${d.name}</option>`;
        });
        document.querySelectorAll('.active-site-dropdown').forEach(drop => drop.innerHTML = activeHtml); document.querySelectorAll('.report-site-dropdown').forEach(drop => drop.innerHTML = allHtml);
        renderSitesTable();
    });
    ...
    onSnapshot(collection(db, "payments"), (snap) => {
        ...
        document.getElementById('dash-paid').innerText = monthlyPaid.toLocaleString(undefined, {minimumFractionDigits: 2}); document.getElementById('dash-cheques').innerText = pendingChequesTotal.toLocaleString(undefined, {minimumFractionDigits: 2});
    ...
    onSnapshot(collection(db, "lorry_cash"), (snap) => {
        ...
        allLorryRecords.reverse(); renderLorryTable();
    });
}
...
function calculateDashboardWidgets() {
    ...
    dashboardStats = {}; let totalDueAll = 0; let totalPaidAll = 0; let totalChequesAll = 0; const currentMonth = new Date().toISOString().slice(0, 7);
    ...
    document.getElementById('dash-due').innerText = totalDueAll.toLocaleString(undefined, {minimumFractionDigits: 2}); document.getElementById('dash-paid').innerText = totalPaidAll.toLocaleString(undefined, {minimumFractionDigits: 2}); document.getElementById('dash-cheques').innerText = totalChequesAll.toLocaleString(undefined, {minimumFractionDigits: 2});
...
document.querySelectorAll('.clickable-card').forEach(card => {
    card.addEventListener('click', (e) => {
        const action = e.currentTarget.dataset.action; if (!action) return; 
        ...
        document.getElementById('db-modal-title').innerText = title; document.getElementById('db-modal-header').className = `modal-header text-white ${headerColor}`;
        if(action === 'CHEQUES') document.getElementById('db-modal-header').classList.replace('text-white', 'text-dark'); 
        let html = ''; let list = Object.values(dashboardStats).filter(v => v[targetField] > 0).sort((a, b) => b[targetField] - a[targetField]);
        ...
        document.getElementById('db-modal-table').innerHTML = html; new bootstrap.Modal(document.getElementById('dashboardBreakdownModal')).show();
    });
});
...
document.getElementById('bill-form').addEventListener('submit', async (e) => {
    ...
    // site_id could be empty (optional now)
...
function renderBillsTable() {
    const bTable = document.getElementById('b-table'); bTable.innerHTML = ''; const state = paginationStates.bills; let dataToRender = currentFilteredBills;
    if (state.pageSize !== 'ALL') { const start = (state.currentPage - 1) * state.pageSize; dataToRender = currentFilteredBills.slice(start, start + state.pageSize); }
    dataToRender.forEach(d => {
...
function renderChequesTable() {
    ...
    const state = paginationStates.cheques; let dataToRender = currentChequesList;
    if (state.pageSize !== 'ALL') { const start = (state.currentPage - 1) * state.pageSize; dataToRender = currentChequesList.slice(start, start + state.pageSize); }
...
document.getElementById('report-type').addEventListener('change', (e) => {
    const isVendor = e.target.value === 'VENDOR'; document.getElementById('report-site-div').style.display = isVendor ? 'none' : 'block'; document.getElementById('report-vendor-div').style.display = isVendor ? 'block' : 'none';
